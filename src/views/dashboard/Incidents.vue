<template>
  ...
  <!-- Review Modal -->
    ...
    >    
      <b-row >
        <b-col cols="12">
          <div @click="toggleAns(ans1Visible)" v-b-toggle.review1 class="card">
            <div class="card-header py-1">
                <h4 class=" mb-0">Supervisor Review</h4>
                <h4 class="text-muted mb-0">1 / 4</h4>
            </div>
          </div>
            <b-collapse  id="review1"  v-model="ans1Visible">
                ...
                <b-row>
                  ...
                  <b-col cols="10" offset="1">
                    <b-form-group
                      ...
                      label-for="sup-name"
                    >
                        <input id="ans1-y" name="ans1" type="radio" value="Yes" style="display:none;"/>
                        <label for="ans1-y" class="btn btn-secondary mr-1" >Yes</label>
                        <input id="ans1-n" name="ans1" type="radio" value="No" style="display:none;"/>
                        <label for="ans1-n" class="btn btn-secondary mr-1" >No</label>
                    </b-form-group>
                  </b-col>
                </b-row>
              </div>
          ...
          <div @click="toggleAns(ans2Visible)" v-b-toggle.review2 class="card">
            <div class="card-header py-1">
                <h4 class=" mb-0">Supervisor Investigation</h4>
                <h4 class="text-muted mb-0">2 / 4</h4>
            </div>
          </div>
          <b-collapse  id="review2" v-model="ans2Visible">
            <div class="card my-card">
              <b-row>
                <b-col cols="10" offset="1" class="pt-1">
                  <b-form-group
                ...
                <b-col cols="10" offset="1">
                  <b-form-group
                      label="Is There Any Witnesse?"
                      label-for="sup-name"
                    >
                        <input id="ans2-y" name="ans2" type="radio" value="Yes" style="display:none;"/>
                        <label @click="showAns2A1 = true" for="ans2-y" class="btn btn-secondary mr-1" >Yes</label>
                        <input id="ans2-n" name="ans2" type="radio" value="No" style="display:none;"/>
                        <label @click="showAns2A1 = false" for="ans2-n" class="btn btn-secondary mr-1" >No</label>
                    </b-form-group>
                </b-col>
                <b-col v-if="showAns2A1" cols="10" offset="1">
                  <b-form-group
                      label="Is it Employee or Visitor?"
                      label-for="sup-sub-name"
                    >
                        <input id="ans2-12-y" name="ans2-12" type="radio" value="Yes" style="display:none;"/>
                        <label for="ans2-12-y" class="btn btn-secondary mr-1" >Employee</label>
                        <input id="ans2-12-n" name="ans2-12" type="radio" value="No" style="display:none;"/>
                        <label for="ans2-12-n" class="btn btn-secondary mr-1" >Visitor</label>
                  </b-form-group>
                </b-col>
              </b-row>
            </div>
          ...
          <div @click="toggleAns(ans3Visible)" v-b-toggle.review3  class="card">
            <div class="card-header py-1">
                <h4 class=" mb-0">Equipments</h4>
                <h4 class="text-muted mb-0">3 / 4</h4>
            </div>
          </div>
          <b-collapse  id="review3" v-model="ans3Visible">
            <div class="card my-card">
              <b-row>
                <b-col cols="10" offset="1" class="pt-1">
                  <b-form-group
          ...
          <div @click="toggleAns(ans4Visible)" v-b-toggle.review4  class="card">
            <div class="card-header py-1">
                <h4 class=" mb-0">Injury</h4>
                <h4 class="text-muted mb-0">4 / 4</h4>
            </div>
          </div>
          <b-collapse  id="review4" v-model="ans4Visible">
            <b-card>
              <b-row>
                <b-col cols="10" offset="1" class="pt-1">
                  <b-form-group
                    label-for="incident-injury"
                    >
                    <v-select
                      id="incident-injury"
                      label="name"
...
<div class="wrapper">
  <canvas id="signature-pad" class="signature-pad" width=400 height=200></canvas>
    ...
    >    
        <b-row @click="setMiddleState" class="justify-content-around">
            <button class="btn btn-outline-warning" data-state="Investigation">Investigation</button>
            <button class="btn btn-outline-danger" data-state="Injury"> Injury </button>
        ...
        <b-row>
          <b-col cols="3">
            <b-card title="Incident Details">
              ...
              <p>{{clickedRow.date}}</p>
              <h5 class="card-subtitle text-muted mt-1">Employee involved</h5>
              <p>{{clickedRow.createdBy.name}}</p>
              <h5 class="card-subtitle text-muted mt-1">Category</h5>
              <p>{{clickedRow.category.name}}</p>
              <h5 class="card-subtitle text-muted mt-1">Location</h5>
              <p>{{clickedRow.location.name}}</p>
            </b-card>
          </b-col>
          <b-col cols="6">
            <b-card title="Incident Management" class="mb-1">
              ...
              <b-tabs>
                <b-tab title="Supervisor Review">
                  <div class="d-flex justify-content-end">
                    <button @click="$bvModal.show('modal-review')" 
                ...
                <b-tab v-if="showInvestigationTabs" title="Investigation">
                  <h1>koko</h1>
                </b-tab>
                <b-tab v-if="showInjuryTabs" title="Medical Certificate">
                  <h1>koko</h1>
                </b-tab>
                <b-tab v-if="showInjuryTabs" title="RTW Plan">
                  <h1>koko</h1>
                </b-tab>
              </b-tabs>
            </b-card>
            ...
            <b-card title="More">
              ...
              <div v-b-toggle.action v-if="showAction == true"  class="card mb-1">
                <div class="d-flex justify-content-between align-items-center">
                  <div class="d-flex align-items-center">
                    <b-avatar src="/11-small.png" class="mr-50"></b-avatar>
                    <div class="detail">
...
import { employeesQuery, postAction,supNamesQuery, incidentsQuery, url } from "@core/query"
...
export default {
  ...
  data() {
    return {
      ...
      showAns2A1: false,
      showAns3a1: false,
      incidentTypes: [{name: 'Health'}, {name: 'Security'}, {name: 'Envirement'}, {name: 'Quality'}],
      incidentType: {name: 'Select a Type'},
      ...
      clickedRow: null,
      middleState: null,
      showInjuryTabs: false,
      showInvestigationTabs: false,
      supNames: [],
      showReview: false,
      signaturePad: null,
    }
  ...
  methods:{
    ...
    const img = Math.floor(1 + Math.random()*12)
    return (
      <div class="d-flex align-items-center">
        <b-avatar src={require(`@/assets/images/avatars/${img}-small.png`)} class="mr-50"></b-avatar>
        <div class="detail">
    ...
    setMiddleState(event) {
      if(event.target.tagName == 'BUTTON') {
        ...
        if(this.middleState == 'Injury') this.showInjuryTabs = true
        if(this.middleState == 'Investigation') this.showInvestigationTabs = true
        this.$bvModal.hide('modal-select-state')
      }
    },
    onRowClick(params){
      this.clickedRow = params.row
      this.$bvModal.show('modal-incident')
    },
    nextState() {
      if(this.clickedRow.state.name == 'Logged') return this.clickedRow.state.name = 'Review'
      else if(this.clickedRow.state.name == 'Review') return this.$bvModal.show('modal-select-state')
